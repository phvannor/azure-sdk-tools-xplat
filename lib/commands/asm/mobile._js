...
exports.init = function (cli) {
  ...
  function getRPWebResource(options) {
    // Create HTTP transport objects
    var currentSubscription = profile.current.getSubscription(options.subscription),
        httpRequest = new WebResource()
         .withHeader('x-ms-version', '2014-01-01');
         //.withHeader('Accept', 'application/json');

    options.subscription = currentSubscription.id;

    httpRequest.uri = currentSubscription.managementEndpointUrl + '/' + options.subscription + '/services/mobileservices';
  ...
  function createClient(options) {
    var currentSubscription = profile.current.getSubscription(options.subscription),
        client = azureCommon.requestPipeline.create(
          azureCommon.SigningFilter.create(currentSubscription._createCredentials()),
          log.createLogFilter(),
          azure.UserAgentFilter.create(utils.getUserAgent()),
  ...
  function getBaseChannel(options) {
    var currentSubscription = profile.current.getSubscription(options.subscription);
    options.subscription = currentSubscription.id;
    var managementEndpoint = url.parse(currentSubscription.managementEndpointUrl);
    var channelConfig = {
      host: managementEndpoint.hostname,
      port: managementEndpoint.port
    };

    if (!currentSubscription.accessToken) {
      var pem = currentSubscription.managementCertificate;
      channelConfig.key = pem.key;
      channelConfig.cert = pem.cert;
    }

    var channel = new Channel(channelConfig).header('x-ms-version', '2012-03-01');
    if (currentSubscription.accessToken) {
      channel = channel.header('Authorization', util.format('Bearer %s', currentSubscription.accessToken.accessToken));
    }

    channel = channel.path(options.subscription);
    return channel;
  }
  ...
  mobile.getRegions = function (options, callback) {
    var client = createClient(options),
        webResource = getRPWebResource(options);

    webResource.uri += '/regions';

    client(webResource, function (error, response, body) {
        callback(error, body);        
    });    
  };
  ...
  mobile.listServices = function (options, callback) {
    ...
    webResource.uri += '/mobileservices';
  ...
  mobile.getService = function (options, callback) {
    ...
    webResource.uri += '/mobileservices/' + options.servicename;
  ...
  mobile.setScaleSettings = function(options, settings, callback) {
    ...
    channel.send('PUT', JSON.stringify(settings), callback);
  };
  ...
  */
  ...
  mobile.register = function(options, _) {
    ...
    try {
      subscription.registerProvider('mobileservice', _);
    }
    finally {
      progress.end();
    }
